...
NORMALIZE_BEFORE_CCA = False
GET_CORRELATIONS = True

dict_out={}
arr_label_corrs_only = np.full((len(score_cols_dict.keys()), len(score_cols_dict.keys())), np.nan)
for i, score_col_ in enumerate(score_cols_dict.values()):
    score_col_ = score_col_[0]
    for j, score_col_2 in enumerate(score_cols_dict.values()):
        score_col_2 = score_col_2[0]
        if i == j:
            arr_label_corrs_only[i, j] = 1
            continue
        s_1 = df_features[score_col_]
        s_2 = df_features[score_col_2]
        mask = s_1.notna() & s_2.notna()
        if mask.sum() == 0:
            continue
        corr, p_value = stats.pearsonr(s_1[mask], s_2[mask])
        arr_label_corrs_only[i, j] = corr

np.save("figure_3_neural_corr/corr_labels_across_scores.npy", arr_label_corrs_only)
