...
# compute_ml(df_features, "psd", False, "XGB", False, True)
...
results = Parallel(n_jobs=-1)(delayed(compute_ml)(df_features, col_score,
                                                  feature_mod, model_type, loc)
                              for model_type in ML_models for loc in locs for feature_mod in feature_mods)
pd.concat([pd.DataFrame(res) for res in results], ignore_index=True)
